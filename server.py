...
def send_cours(id):
    db_sess = db_session.create_session()
    user = db_sess.query(Courses).filter(Courses.id == id).first()
    smtpObj = smtplib.SMTP('smtp.mail.ru', 587)
    smtpObj.starttls()
    smtpObj.login('hellomirkrutit@mail.ru', 'Q?ijggOi')
    smtpObj.sendmail("hellomirkrutit@mail.ru", current_user.email,
                     f"Ссылка на файлы вашего курса: {user.url_on_files}")
    smtpObj.quit()
